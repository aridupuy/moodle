 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class webservice_tiendanube_server_testcase extends advanced_testcase {
    ...
    public function test_get_headers() {
        ...
        $builder = $this->getMockBuilder('webservice_tiendanube_server');
        $builder->disableOriginalConstructor();
        $stub = $builder->getMock();

        // We're testing a private method, so we need to setup reflector magic.
        $method = new ReflectionMethod('webservice_tiendanube_server', 'get_headers');
    ...
    public function test_get_wstoken() {
        ...
        // We're testing a private method, so we need to setup reflector magic.
        $method = new ReflectionMethod('webservice_tiendanube_server', 'get_wstoken');
        $method->setAccessible(true); // Allow accessing of private method.
        $proxy = $method->invoke($stub, $headers); // Get result of invoked method.
    ...
    public function test_get_wstoken_error() {
        ...
        $method = new ReflectionMethod('webservice_tiendanube_server', 'get_wstoken');
        $method->setAccessible(true); // Allow accessing of private method.
        $proxy = $method->invoke(new webservice_tiendanube_server(WEBSERVICE_AUTHMETHOD_PERMANENT_TOKEN), $headers);
    }
    ...
    public function test_get_wsfunction() {
        ...
        // We're testing a private method, so we need to setup reflector magic.
        $method = new ReflectionMethod('webservice_tiendanube_server', 'get_wsfunction');
        $method->setAccessible(true); // Allow accessing of private method.
    ...
    public function test_get_wsfunction_error() {
        ...
        $method = new ReflectionMethod('webservice_tiendanube_server', 'get_wsfunction');
        $method->setAccessible(true); // Allow accessing of private method.
        $proxy = $method->invoke(new webservice_tiendanube_server(WEBSERVICE_AUTHMETHOD_PERMANENT_TOKEN), $getvars);
    }
    ...
    public function test_get_responseformat() {
        ...
        // We're testing a private method, so we need to setup reflector magic.
        $method = new ReflectionMethod('webservice_tiendanube_server', 'get_responseformat');
        $method->setAccessible(true); // Allow accessing of private method.
        $proxy = $method->invoke($stub, $headers); // Get result of invoked method.
    ...
    public function test_get_requestformat() {
        ...
        // We're testing a private method, so we need to setup reflector magic.
        $method = new ReflectionMethod('webservice_tiendanube_server', 'get_requestformat');
        $method->setAccessible(true); // Allow accessing of private method.
        $proxy = $method->invoke($stub, $headers); // Get result of invoked method.
